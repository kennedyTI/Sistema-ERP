...
def portal_permissions_for_groups(groups: list[str], *, is_superuser: bool = False) -> PortalPermissions:
    ...
    if is_superuser or GROUP_EQUIPE_TECNICA in group_set:
        return PortalPermissions(can_access_portal=True, can_access_admin=True)

    if group_set & PORTAL_GROUPS:
        return PortalPermissions(can_access_portal=True, can_access_admin=False)
